...
val mountPoint = s"/mnt/etlp1s-$username-ss"
...
try {
  val MountTarget = "s3a://%s:%s@%s".format(awsAccessKey, secretKey, awsBucketName)
  dbutils.fs.mount(MountTarget, mountPoint)
} catch {
  case ioe: java.rmi.RemoteException => println($"$mountPoint already mounted. Run previous cells to unmount first")
}
...
print("Hint: Your mount name is %s".format(mountPoint))
